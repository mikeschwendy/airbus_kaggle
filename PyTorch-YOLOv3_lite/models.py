...
def create_modules(module_defs):
    ...
    for i, module_def in enumerate(module_defs):
        ...
        if module_def['type'] == 'convolutional':
            ...
            filters = int(module_def['filters'])
            modules.add_module('connected', nn.Linear(in_features=output_filters[-1], 
...
class Darknet_Classifier(nn.Module):
    """YOLOv3 object classifier model"""
    def __init__(self, config_path, img_size=416):
        super(Darknet_Classifier, self).__init__()
        self.module_defs = parse_model_config(config_path)
        self.hyperparams, self.module_list = create_modules(self.module_defs)
        self.img_size = img_size
        self.seen = 0
    ...
    def forward(self, x, targets=None):
        ...
        for i, (module_def, module) in enumerate(zip(self.module_defs, self.module_list)):
            ...
            layer_outputs.append(x)
        return loss if is_training else x
