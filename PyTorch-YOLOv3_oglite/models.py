...
from utils.utils import build_targets, bbox_iou
...
class YOLOLayer(nn.Module):
    ...
    def forward(self, x, targets=None):
        ...
        if targets is not None:
            ...
            output = torch.cat(
                (
                    pred_boxes.view(nB, -1, 4) * stride * 768/416,
                    pred_conf.view(nB, -1, 1)                ),
                -1,
            )
            #pdb.set_trace()
...
class Darknet(nn.Module):
    ...
    def load_weights(self, weights_path, max_layer=None):
